...
average_delay_15 <- flights %>% 
  summarise(mean = mean(arr_delay))

```
#can flights be both? how do you parse out only 50% of the time?
...
#i suppose my main question is how do you parse out the probabilities (this was the only hard part of statistics) and then type code to take each into consideration?
...
my_not_cancelled <- not_cancelled %>% 
  n <- #i want to add up all flights for each destination
  select(flights, "dest", "n")

not_cancelled %>% count(tailnum, wt = distance)
#what is the frequency weight (wt)? does it add up all the distances?
```
...
Could you elicit the same effect with is.na(arr_time)?
...
I feel confused in figuring out where I need to start with problems like this.
